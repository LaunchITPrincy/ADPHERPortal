...
import pages.ProgramMedicalPhysicistPage;
import utils.ExtentReports.ExtentTestManager;
import utils.Helper.Config;
import utils.Helper.ExcelUtil;

public class ApplyProgramMedicalPhysicistPage extends BaseTest{
	...
	public void VerifyProgramDropdownValues(Method method) {
		...
		page.ClickProgram();
		ProgramMedicalPhysicistPage medical = new ProgramMedicalPhysicistPage(driver);
		medical.AssertEquals_ProgramDropd();	
		ExtentTestManager.endTest();
	}
	
	@Test(priority = 4, description = "Verify Medical Physicist Program Detail Field Dropdown Values", enabled = false)
	public void VerifyMedicalPhysicistProgramDetailsDropdownValues(Method method) {

		ExtentTestManager.startTest(method.getName(),
				"Verify Medical Physicist Program Detail dropdown values in the Apply Program Page");
		ExcelUtil.setCurrentRow(28, 3);
		ProgramMedicalPhysicistPage medical = new ProgramMedicalPhysicistPage(driver);
		medical.MedicalPhysicist_ProgramDetail();
		ExtentTestManager.endTest();
	}
