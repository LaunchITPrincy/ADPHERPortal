...
import pages.ProgramPlumbingandMechanicalSystemsPage;
import utils.ExtentReports.ExtentTestManager;
import utils.Helper.Config;
import utils.Helper.ExcelUtil;

public class ApplyProgramPlumbingandMechanicalSystems extends BaseTest	{
	...
	public void VerifyProgramDropdownValues(Method method) {
		...
		page.ClickProgram();
		ProgramPlumbingandMechanicalSystemsPage mech = new ProgramPlumbingandMechanicalSystemsPage(driver);
		mech.AssertEquals_ProgramDropd();	
		ExtentTestManager.endTest();
	}
	
	@Test(priority = 4, description = "Verify Plumbing and Mechanical Systems Program Detail Field Dropdown Values", enabled = false)
	public void VerifyPlumbingandMechanicalSystemsProgramDetailsDropdownValues(Method method) {

		ExtentTestManager.startTest(method.getName(),
				"Verify Plumbing and Mechanical Systems Program Detail dropdown values in the Apply Program Page");
		ExcelUtil.setCurrentRow(28, 3);
		ProgramPlumbingandMechanicalSystemsPage mech = new ProgramPlumbingandMechanicalSystemsPage(driver);
		mech.PlumbingandMechanicalSystems_ProgramDetail();
		ExtentTestManager.endTest();
	}
